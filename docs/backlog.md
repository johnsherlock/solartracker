...
| Q-009 | Deployment / Operations | Fix GitHub workflows for the `v2` rewrite line | Replace the legacy-root CI assumptions with workflows that validate the rewrite app on `v2`, and remove legacy-only code from `v2` once it is no longer needed for the rewrite. | PRs targeting `v2` run the rewrite app test/build checks; pushes to `v2` run the same checks; legacy-root build assumptions are removed from `v2` workflows; legacy-only app code and tooling are removed from `v2` so the branch reflects the rewrite product only; workflow scope is documented in-repo. | P-017, Q-001a | Todo | Immediate cleanup item now that `v2` is the default branch and `main` is the legacy line. |
