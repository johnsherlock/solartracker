...
- The legacy V1 MyEnergi proxy has a known DST-boundary quirk: it adjusts `hr`/`min` for timezone but can leave `dom`/`mon`/`yr` on the previous local day for the leading 00:00-00:59 block. Any future direct rewrite-owned MyEnergi adapter must normalize full timestamps at the adapter layer and must not trust partially shifted local date fields from this legacy proxy behavior.
